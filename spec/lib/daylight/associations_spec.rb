...
describe Daylight::Associations do
  ...
  class AssociationsTestClass < Daylight::API
    ...
    remote     :remote_stuff, class_name: 'RelatedTestClass', http_method: :get
  ...
  class AnotherAssociationTestClass < Daylight::API
    belongs_to :parent,   class_name: 'RelatedTestClass'
    remote :patch_remote, class_name: 'AnotherAssociationTestClass', http_method: :patch
  end
  ...
  describe :remote do
    ...
    context 'get' do
      def respond_with(http_method, data)
        stub_request(http_method, %r{#{AssociationsTestClass.site}}).to_return(body: data.to_json)
      end
    ...
    context 'patch' do
      def respond_with(http_method, data)
        stub_request(http_method, %r{#{AnotherAssociationTestClass.site}}).to_return(:body => data.to_json)
      end
