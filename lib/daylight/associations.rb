...
module Daylight::Associations
  ...
  module ClassMethods
    ...
    # Example:
    #
    #   remote :posts_by_popularity, class_name: 'post', http_method: :get
    #
    #   or
    #
    #   remote :posts_by_popularity, class_name: 'post', http_method: :patch
    #
    ...
    def remote name, options
      create_reflection(:remote, name, options).tap do |reflection|
        define_cached_method(reflection.name) do |args=nil|
          call_remote(reflection.name, reflection.klass, reflection.options[:http_method], args)
        end
      end
    end
